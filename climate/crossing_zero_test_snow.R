...
Bakhta_daily <- read.csv2("initial_data/climate/cleaned/Bakhta_daily_attention1966-1976(problems_with_summer).csv") |>
  filter(Year > 1976) # Так как данные суточного разрешения есть только с 1977 года
...
spring_data <- only_march_may |>
  ...
  group_by(Year) |>
  mutate(
    more_then_zero = (Sn > 1), # логический столбец, глубина снега больше 5? (TRUE | FALSE)
...
write.csv2(crossing_zero, "initial_data/climate/cleaned/start_end_of_spring_autumn_accroding_snow_cover.csv")
...
# Вычислим продолжительность зимнего периода ((Winter_2(Year) + Winter_1(Year+1))
by_winter <- by_season |>
  ungroup() |>
  filter(Season %in% c("Winter_1", "Winter_2")) |>
  pivot_wider(names_from = Season, values_from = c(Duration_of_season)) |>
  mutate(
    Winter_2_previous = lag(Winter_2)
  ) |>
  filter(!(Year %in% c(1977, 2023))) |>
  mutate(
    Duration_of_season = Winter_1 + Winter_2_previous
  ) |>
  mutate(Season = "Winter") |>
  select(Year, Season, Duration_of_season)
# Результат ---------------------------------------------------------------
...
result_for_modelling <- result |>
  pivot_wider(names_from = Season, values_from = c(Duration), names_prefix = "Snow_duration_")

write.csv2(result, "initial_data/climate/cleaned/Duration_of_season(snow)_for_graph.csv")
write.csv2(result_for_modelling, "initial_data/climate/cleaned/Duration_of_season(snow)_for_modelling.csv")
